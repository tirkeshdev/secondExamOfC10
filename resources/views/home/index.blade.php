    ...
    @foreach($categoryBooks as $categoryBook)
        <div class="container-xl">
            <div class="display-4 text-danger pt-5">
                <a href="{{ route('book.index', ['categories' => [$categoryBook['category']->id]]) }}"
                   class="link-danger text-decoration-none">
                    {{ $categoryBook['category']->getname() }}
                </a>
                <span class="text-warning h4 fw-normal">({{$categoryBook['category']->books_count}} @lang('app.books')
                    )</span>
            </div>
                        ...
                        <a href="{{route('details', $book->id)}}" class="text-decoration-none link-danger">
                            <div class="card bg-dark text-danger border-danger"
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="card-text pb-3">{{ round($book->price, 2) }}
                                        <small>TMT</small>
                                    </div>
                                    @auth
